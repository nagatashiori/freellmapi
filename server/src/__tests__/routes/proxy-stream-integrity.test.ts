...
function mockUpstream(script: Array<{ body: string; status?: number }>) {
  ...
  vi.spyOn(global, 'fetch').mockImplementation(async (url, init) => {
    ...
    if (!/api\.groq\.com|openrouter\.ai|api\.cohere|generativelanguage|integrate\.api\.nvidia|api\.cerebras|api\.mistral|router\.huggingface|api\.cloudflare|models\.github|open\.bigmodel|api\.llm7|api\.kilo|text\.pollinations|ollama\.com|opencode\.ai|api\.aionlabs\.ai|router\.requesty\.ai|router\.bynara\.id/.test(urlStr)) {
