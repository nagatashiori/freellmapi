...
// validateUrl: inference base has NO /models (404); catalog is separate.
register(new OpenAICompatProvider({
    platform: 'github',
    name: 'GitHub Models',
...
export function isValidUserPlatformSlug(slug) {
    return /^[a-z][a-z0-9_-]{0,31}$/.test(slug);
}
export function isReservedPlatformSlug(slug) {
    return RESERVED_PLATFORM_SLUGS.has(slug) || providers.has(slug);
}
export function rememberUserPlatform(slug) {
    const s = slug.trim().toLowerCase();
    if (!isValidUserPlatformSlug(s) || isReservedPlatformSlug(s))
        return;
    userPlatforms.add(s);
}
export function isUserPlatform(platform) {
...
export function hydrateUserPlatformsFromDb(db) {
    try {
        const rows = db.prepare(`SELECT DISTINCT platform FROM api_keys
        WHERE base_url IS NOT NULL AND TRIM(base_url) != ''`).all();
        for (const r of rows) {
...
export function resolveProvider(platform, baseUrl) {
    ...
    if (!providers.has(p) && baseUrl?.trim()) {
...
export function hasProvider(platform) {
    const p = String(platform);
    return providers.has(p) || p === 'custom' || userPlatforms.has(p);
}
