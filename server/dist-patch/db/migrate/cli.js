...
function updateDefaultMigrationRegistry(filename) {
    ...
    if (registry.includes(importLine) || registry.includes(entryLine))
        return;
...
function sanitiseMigrationName(name) {
    return name
        .toLowerCase()
        .replace(/[ -]+/g, '_')
        .replace(/[^a-z0-9_]/g, '')
...
function toMigrationBindingName(filename) {
