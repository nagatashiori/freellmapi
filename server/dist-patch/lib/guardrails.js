...
export function getRequestMaxTokensBudget() {
...
export function getMaxConsecutiveUpstreamFails() {
 ...
 */
export function applyTokenBudget(estimatedInputTokens, maxTokens) {
    ...
    if (estimatedTotal > budget)
        return { rejection: { budget, estimatedTotal }, maxTokens };
    if (maxTokens == null) {
...
export function tokenBudgetMessage(r) {
...
export function newBreaker(limit = getMaxConsecutiveUpstreamFails()) {
 ...
 */
export function recordBreakerFailure(state) {
    if (state.limit <= 0)
        return false;
    state.consecutive += 1;
