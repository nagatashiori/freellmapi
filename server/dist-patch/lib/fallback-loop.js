...
import { recordRequest, recordTokens, setCooldown, getCooldownDurationForLimit, getSoonestCooldownExpiry, PAYMENT_REQUIRED_COOLDOWN_MS, MODEL_FORBIDDEN_COOLDOWN_MS, learnLimitFromError, } from '../services/ratelimit.js';
import { isRetryableError, isKeyAuthError, isDailyQuotaExhaustedError, isPaymentRequiredError, isModelNotFoundError, isModelAccessForbiddenError, isProviderBadRequestError, } from './error-classify.js';
...
export function getFallbackTimeBudgetMs() {
    let stored;
    try {
...
export function newFallbackState() {
    return { skipKeys: new Set(), skipModels: new Set() };
}
 ...
 */
export function cooldownForError(route, err) {
    ...
    if (isDailyQuotaExhaustedError(err))
        return err?.retryAfterMs ?? msUntilNextUtcMidnight();
 ...
 */
export function recordRetryableFailure(route, err, state) {
...
function triggerKeyRevalidation(platform, keyId) {
    const now = Date.now();
    const last = lastRevalidation.get(keyId) ?? 0;
    if (now - last < REVALIDATION_DEDUPE_MS)
        return;
 ...
 */
export function recordAuthFailure(route, state) {
    state.skipKeys.add(`${route.platform}:${route.modelId}:${route.keyId}`);
 ...
 */
export function recordUpstreamSuccess(route, rateLimitTokens) {
    recordRequest(route.platform, route.modelId, route.keyId);
    recordTokens(route.platform, route.modelId, route.keyId, rateLimitTokens);
    recordSuccess(route.modelDbId);
}
export function classifyAttemptError(err) {
    ...
    if (isModelAccessForbiddenError(err))
        return 'forbidden';
    if (isProviderBadRequestError(err))
        return 'provider_bad_request';
    const msg = (err?.message ?? '').toLowerCase();
    if (msg.includes('empty completion'))
        return 'empty_completion';
    if (msg.includes('ignored response_format') || msg.includes('truncated json'))
        return 'format_ignored';
    if (msg.includes('timeout') || msg.includes('stalled') || msg.includes('etimedout') || msg.includes('aborted'))
        return 'timeout';
    if (msg.includes('429') || msg.includes('rate limit') || msg.includes('too many requests') || msg.includes('quota'))
        return 'rate_limited';
    const status = typeof err?.status === 'number' ? err.status : 0;
    if (status >= 500 || msg.includes('500') || msg.includes('502') || msg.includes('503') || msg.includes('unavailable') || msg.includes('internal server error'))
        return 'upstream_error';
    return 'error';
}
const TRAIL_MAX_SHOWN = 10;
export function formatAttemptTrail(attempts) {
 ...
 */
export function setFallbackHeaders(res, failedAttempts, trail) {
    if (failedAttempts > 0)
        res.setHeader('X-Fallback-Attempts', String(failedAttempts));
 ...
 */
export function exhaustedRetryError(lastError, maxRetries, ctx) {
 ...
 */
export async function runFallbackLoop(hooks) {
    const maxRetries = hooks.maxRetries ?? FALLBACK_MAX_RETRIES;
    const budgetMs = hooks.timeBudgetMs ?? getFallbackTimeBudgetMs();
    const startedAt = Date.now();
    const attempts = hooks.attemptLog ?? [];
    const keyOrdinals = new Map();
    const keyOrdinal = (route) => {
    ...
    const stopIfBreakerTripped = () => {
        if (!recordBreakerFailure(breaker))
            return false;
        ...
        hooks.onExhausted(exhaustedRetryError(lastError, maxRetries, { attempts, breakerFails: breaker.consecutive }), { attempts, timedOut: false });
        return true;
    };
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        ...
        if (attempt > 0 && budgetMs > 0 && Date.now() - startedAt >= budgetMs) {
            hooks.onExhausted(exhaustedRetryError(lastError, maxRetries, { attempts, timedOut: true, budgetMs }), { attempts, timedOut: true });
            return;
        }
        let route;
        try {
        ...
        try {
            ...
            if (isRetryableError(err)) {
                ...
                if (err?.skipBench !== true && stopIfBreakerTripped())
                    return;
                continue;
            }
        ...
        if (outcome !== 'done' && outcome !== 'committed') {
            const violation = new Error(`fallback-loop dispatch contract violation on ${route.platform}/${route.modelId}: ` +
    ...
    hooks.onExhausted(exhaustedRetryError(lastError, maxRetries, { attempts }), { attempts, timedOut: false });
}
//# sourceMappingURL=fallback-loop.js.map