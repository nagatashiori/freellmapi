...
export function isDbBackupConfigured() {
...
async function readTarget(target) {
    if (isHttpTarget(target)) {
        ...
        if (res.status === 404 || res.status === 204)
            return null;
        if (!res.ok)
            throw new Error(`backup restore failed: HTTP ${res.status}`);
...
async function writeTarget(target, payload) {
    if (isHttpTarget(target)) {
        ...
        if (!res.ok)
            throw new Error(`backup upload failed: HTTP ${res.status}`);
        return;
    }
    fs.mkdirSync(path.dirname(path.resolve(target)), { recursive: true });
    fs.writeFileSync(target, payload);
}
export async function restoreDbBackupIfNeeded(dbPath = getDefaultDbPath()) {
    const target = backupTarget();
    if (!target)
        return { ok: true, skipped: 'not configured' };
    if (dbPath === ':memory:')
        return { ok: true, target, skipped: 'memory database' };
    if (fs.existsSync(dbPath) && fs.statSync(dbPath).size > 0)
        return { ok: true, target, skipped: 'database already exists' };
    const payload = await readTarget(target);
    if (!payload || payload.length === 0)
        return { ok: true, target, skipped: 'no backup found' };
    let restored;
    try {
...
export async function backupDbNow(db, dbPath = getDefaultDbPath()) {
    const target = backupTarget();
    if (!target)
        return { ok: true, skipped: 'not configured' };
    if (dbPath === ':memory:')
    ...
    if (!fs.existsSync(dbPath))
        return { ok: false, target, skipped: 'database file missing' };
    try {
...
export function startDbBackupPump(db, scheduler, dbPath = getDefaultDbPath()) {
    if (!backupTarget())
        return null;
    const run = () => {
