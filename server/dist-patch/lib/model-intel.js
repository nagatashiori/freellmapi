 ...
 */
export function calibrateModelMeta(modelId, displayName) {
...
export function niceDisplayName(modelId, explicit) {
    if (explicit?.trim())
        return explicit.trim();
    const id = modelId.trim();
    const base = id.split('/').pop() || id;
    if (/k2\.6|k2-6|kimi-k2\.6/i.test(id))
        return 'Kimi K2.6';
    if (/k2\.7|k2-7|kimi-k2\.7/i.test(id))
        return 'Kimi K2.7';
    ...
    if (/mistral-large/i.test(id))
        return 'Mistral Large';
    if (/minimax.*m2\.7|m2\.7/i.test(id))
        return 'MiniMax M2.7';
    if (/minimax/i.test(id))
        return 'MiniMax';
    if (/gpt-oss-120/i.test(id))
        return 'GPT-OSS 120B';
    if (/nemotron-3-ultra/i.test(id))
...
//# sourceMappingURL=model-intel.js.map