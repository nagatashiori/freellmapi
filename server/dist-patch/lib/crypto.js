...
function writeKeyFileAtomic(keyFile, hex) {
    ...
    catch { /* best effort — e.g. filesystems without POSIX modes */ }
...
export function initEncryptionKey(db) {
    ...
    const legacy = db.prepare("SELECT value FROM settings WHERE key = 'encryption_key'").get();
    if (legacy) {
...
export function isEncryptionKeyInitialized() {
