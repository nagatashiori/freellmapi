...
export function startsWithDialectMarker(text) {
 ...
 */
export function couldBecomeDialectMarker(text) {
    const t = text.trimStart();
    if (t.length === 0)
        return true;
...
export function containsDialectMarker(text) {
...
function extractBalancedJson(text, from) {
    ...
    for (let i = from; i < text.length; i++) {
        ...
        if (ch === '"')
            ...
            depth--;
            if (depth === 0)
                return { json: text.slice(from, i + 1), end: i + 1 };
        }
    }
    return null;
}
const isKnownTool = (name, toolNames) => toolNames.size === 0 || toolNames.has(name);
/** Parse `{"name": ..., "arguments"|"parameters": ...}` into a call. */
function callFromNamedJson(json, toolNames) {
    let obj;
    try {
        obj = JSON.parse(json);
    }
    ...
    if (!name || !isKnownTool(name, toolNames))
        return null;
...
function parseTokenDialect(text, toolNames) {
    const calls = [];
    let clean = text;
    ...
    while ((m = callRe.exec(clean)) !== null) {
        ...
        if (extracted && name && isKnownTool(name, toolNames)) {
            ...
            if (argsOk)
                calls.push({ name, arguments: extracted.json });
        }
        if (!argsOk)
            parsedAll = false;
...
function parseFunctionTagDialect(text, toolNames) {
    const calls = [];
    let clean = text;
    let parsedAll = true;
    ...
    while ((m = headRe.exec(text)) !== null) {
        ...
        if (!ok)
            parsedAll = false; // array-shaped or invalid args: not a callable shape
...
function parseXmlDialect(text, toolNames) {
    const calls = [];
    let parsedAll = true;
 ...
 *                   used by tests only)
 */
export function rescueInlineToolCalls(text, toolNames) {
    if (!text)
...
//# sourceMappingURL=tool-call-rescue.js.map