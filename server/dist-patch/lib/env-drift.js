...
export function parseEnvNames(text, opts = {}) {
    ...
    for (const line of text.split(/\r?\n/)) {
        ...
        if (opts.includeCommented) {
            ...
            if (commented)
                names.add(commented[1]);
        }
    }
    return [...names].sort();
}
function isKnownUndocumentedEnv(name) {
    return KNOWN_UNDOCUMENTED_ENV.has(name) || KNOWN_UNDOCUMENTED_PATTERNS.some(re => re.test(name));
}
export function compareEnvText(envText, exampleText) {
...
function compactList(names, limit = 8) {
    if (names.length <= limit)
        return names.join(', ');
    return `${names.slice(0, limit).join(', ')} +${names.length - limit} more`;
}
export function formatEnvDriftReport(report) {
    const parts = [];
...
export function checkEnvDrift(paths = {}) {
    ...
    if (!fs.existsSync(envPath) || !fs.existsSync(examplePath))
        return null;
    try {
        return compareEnvText(fs.readFileSync(envPath, 'utf8'), fs.readFileSync(examplePath, 'utf8'));
    }
    catch {
        return null;
    }
}
export function warnOnEnvDrift(paths, logger = console) {
    const report = checkEnvDrift(paths);
    if (!report)
        return null;
    for (const line of formatEnvDriftReport(report))
        logger.warn(line);
    return report;
}
//# sourceMappingURL=env-drift.js.map