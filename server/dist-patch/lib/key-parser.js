...
export const AUTH_JSON_PROVIDER_MAP = {
    gemini: 'google',
    google: 'google',
    groq: 'groq',
...
export function detectPlatform(prefix) {
 ...
 */
export function parseExportJson(content) {
    let parsed;
    try {
        parsed = JSON.parse(content);
    }
    ...
    const keys = obj.keys;
    if (keys.length > 0 && typeof keys[0] === 'object' && keys[0] !== null && 'platform' in keys[0] && 'key' in keys[0]) {
        const result = { keys: [], skipped: [] };
...
export function parseAuthJson(content) {
    let parsed;
    try {
        parsed = JSON.parse(content);
    }
...
export function parseKeysFromFile(content, filename) {
    let text = content;
    if (text.charCodeAt(0) === 0xfeff)
        text = text.slice(1);
    text = text.replace(/\r\n/g, '\n');
