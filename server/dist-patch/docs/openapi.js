...
export const openapiSpec = {
    ...
    info: {
        title: 'FreeLLMAPI',
        version: '0.4.1',
        description: 'OpenAI-compatible proxy that aggregates free LLM provider tiers behind a single /v1 endpoint. ' +
    ...
    paths: {
        '/chat/completions': {
            post: {
                ...
                description: 'OpenAI-compatible chat completions. Set `model` to `auto` (the default) to let the router ' +
                ...
                responses: {
                    '200': {
                        description: 'A chat completion. When `stream: true`, the body is a `text/event-stream` of ' +
        ...
        '/completions': {
            post: {
                ...
                description: 'OpenAI-compatible legacy completions. Editor ghost-text clients still send `prompt`/`suffix` ' +
        ...
        '/images/generations': {
            post: {
                ...
                description: 'OpenAI-compatible image generation, routed through the media catalog. Omit `model` (or set ' +
        ...
        '/audio/speech': {
            post: {
                ...
                description: 'OpenAI-compatible text-to-speech, routed through the media catalog. Returns raw audio bytes; ' +
        ...
        '/responses': {
            post: {
                ...
                description: 'The OpenAI Responses wire format that current Codex CLI versions require, implemented as a ' +
        ...
        '/messages': {
            post: {
                ...
                description: "Anthropic's Messages wire format over the same router, so Claude Code and the official " +
        ...
        '/models': {
            get: {
                ...
                description: 'Lists the catalog, one row per model id, each tagged with whether it is usable right now, ' +
