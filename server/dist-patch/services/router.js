...
function selectKeyForModel(entry, estimatedTokens, skipKeys, diag) {
    ...
    for (let attempt = 0; attempt < keys.length; attempt++) {
        ...
        const resolvedProvider = resolveProvider(entry.platform, key.base_url)
            ?? provider;
        if (!resolvedProvider) {
...
export function hasOtherUsableKey(modelDbId, excludingKeyId, skipKeys) {
  ...
  `).get(modelDbId);
    ...
    const keys = db.prepare("SELECT id FROM api_keys WHERE platform = ? AND enabled = 1 AND status IN ('healthy', 'unknown')").all(m.platform);
    for (const k of keys) {
        ...
        if (m.platform === 'custom' && m.key_id != null && k.id !== m.key_id)
            continue;
...
export function routeRequest(estimatedTokens = 1000, skipKeys, preferredModelDbId, requireVision = false, requireTools = false, skipModels, prefetchedChain, requireStructured = false) {
    const db = getDb();
    const strategy = getRoutingStrategy();
    if (strategy !== 'priority')
        refreshStatsCache(db);
