...
const store = new Map();
export function getContextHandoffMode() {
...
export function recordIncomingMessages(sessionKey, messages) {
    if (!sessionKey)
        return;
    pruneExpired();
    ...
    if (existing) {
        if (!hasAssistant)
            existing.lastModelKey = undefined;
    ...
    if (store.size > MAX_STORE_SIZE) {
        ...
        for (const [k, v] of store) {
            if (now - v.updatedAt > SESSION_TTL_MS)
                store.delete(k);
        }
        if (store.size > MAX_STORE_SIZE) {
...
export function hasPriorModel(sessionKey) {
    if (!sessionKey)
        return false;
...
export function maybeInjectContextHandoff(params) {
    ...
    const handoffMsg = { role: 'system', content: handoffContent };
