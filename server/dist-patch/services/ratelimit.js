...
const nullLimitHits = new Map(); // key -> timestamps
function recordNullLimitHit(platform, modelId, keyId, now) {
    const key = `${platform}:${modelId}:${keyId}`;
    const hits = nullLimitHits.get(key) ?? [];
    hits.push(now);
    nullLimitHits.set(key, hits);
}
function clearNullLimitHits(platform, modelId, keyId) {
    nullLimitHits.delete(`${platform}:${modelId}:${keyId}`);
}
export function recentHitCount(platform, modelId, keyId, now, windowMs = NULL_LIMIT_HIT_WINDOW_MS) {
    const key = `${platform}:${modelId}:${keyId}`;
    const hits = nullLimitHits.get(key) ?? [];
 ...
 */
export function getSoonestCooldownExpiry(now = Date.now()) {
    return withDb(db => {
        const row = db.prepare(`
