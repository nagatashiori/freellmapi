...
const EMPTY_OVERRIDES = { merges: [], splits: [] };
// ── Settings accessors ───────────────────────────────────────────────────────
...
export function isUnifyEnabled() {
    return true;
}
export function setUnifyEnabled(on) {
    setSetting(UNIFY_ENABLED_KEY, on ? '1' : '0');
}
export function getUnifyOverrides() {
...
export function setUnifyOverrides(input) {
...
export function stripProviderSuffix(displayName) {
...
export function normalizeGroupKey(displayName) {
...
function tokenForRow(row, ov) {
    const mid = memberId(row);
    const split = ov.splits.find(s => s.member === mid);
    if (split)
        return split.groupKey ? normalizeGroupKey(split.groupKey) : `__split__:${mid}`;
...
function assignCanonicalIds(groups) {
    const used = new Set();
 ...
 */
export function groupRows(rows, ov) {
    const map = new Map();
    ...
    for (const g of map.values()) {
        const rep = [...g.members].sort((a, b) => (a.intelligence_rank ?? Number.MAX_SAFE_INTEGER) - (b.intelligence_rank ?? Number.MAX_SAFE_INTEGER)
 ...
 */
export function resolveRequestedIdToMembers(requested, groups) {
    if (!requested)
        return null;
    const byCanonical = groups.find(g => g.canonicalId === requested);
    if (byCanonical)
        return byCanonical.members.map(m => m.model_db_id);
