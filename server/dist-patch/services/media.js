...
async function mediaFetch(url, platform, modality, init) {
    const r = await proxyFetch(url, { ...init, signal: AbortSignal.timeout(FETCH_TIMEOUT_MS) }, platform, modality, FETCH_TIMEOUT_MS);
    if (!r.ok) {
...
async function callImageProvider(row, credential, p) {
    const key = credential.key;
    const [w, h] = parseSize(p.size);
    switch (row.platform) {
        case 'custom': {
            if (!credential.baseUrl)
                throw new MediaError('custom image provider is missing base_url', 500);
            const body = { model: row.model_id, prompt: p.prompt };
            if (p.n !== undefined)
                body.n = p.n;
            if (p.size)
                body.size = p.size;
...
async function callSpeechProvider(row, credential, p) {
    const key = credential.key;
    switch (row.platform) {
        case 'custom': {
            if (!credential.baseUrl)
                throw new MediaError('custom audio provider is missing base_url', 500);
            const fmt = p.format ?? 'mp3';
            const body = { model: row.model_id, input: p.input };
            if (p.voice)
                body.voice = p.voice;
            if (p.format)
                body.response_format = p.format;
        ...
        case 'google': {
            ...
            const r = await mediaFetch(`https://generativelanguage.googleapis.com/v1beta/models/${row.model_id}:generateContent?key=${encodeURIComponent(key ?? '')}`, 'google', 'audio', {
                method: 'POST',
...
function logMedia(row, keyId, status, latencyMs, error) {
    try {
