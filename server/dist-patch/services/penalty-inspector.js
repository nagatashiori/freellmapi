...
function rowKey(modelDbId, platform, modelId) {
    return modelDbId != null ? `id:${modelDbId}` : `model:${platform}:${modelId}`;
}
function ensureInspectorRow(rows, input) {
...
export function getPenaltyInspector() {
    const db = getDb();
    const now = Date.now();
    const rows = new Map();
    ...
    `).all(...penalties.map(p => p.modelDbId));
  ...
  `).all(since);
    ...
    const ordered = [...rows.values()].sort((a, b) => b.penalty.value - a.penalty.value ||
...
//# sourceMappingURL=penalty-inspector.js.map