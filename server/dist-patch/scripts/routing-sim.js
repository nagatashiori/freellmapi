...
import { routeRequest, refreshStatsCache, setRoutingStrategy, getRoutingScores, } from '../services/router.js';
...
function seed() {
  ...
  `);
    PROFILES.forEach((p, i) => {
        ...
        const id = db.prepare('SELECT id FROM models WHERE platform=? AND model_id=?').get(p.platform, p.modelId).id;
        insFb.run(id, i + 1);
        ...
        for (let s = 0; s < p.successes; s++)
            insHist.run(p.platform, p.modelId, 'success', p.outTokens, p.latencyMs, null, p.ttfbMs);
        for (let f = 0; f < p.failures; f++)
            insHist.run(p.platform, p.modelId, 'error', 0, p.latencyMs, 'sim-fail', p.ttfbMs);
    });
}
...
function printDistribution(title, counts, runs) {
    console.log(`\n  ${title}`);
...
function printScores() {
    ...
    for (const s of scores) {
        ...
        console.log(`    ${s.displayName.padEnd(22)} ` +
...
main();
//# sourceMappingURL=routing-sim.js.map