...
import { z } from 'zod';
import { routeRequest, resolveRoutingChain, resolveModelGroupCandidates, hasEnabledVisionModel, hasEnabledToolsModel, routingReserveTokens } from '../services/router.js';
...
import { runFallbackLoop, newFallbackState, recordUpstreamSuccess, exhaustedRetryError, setFallbackHeaders } from '../lib/fallback-loop.js';
...
proxyRouter.post('/completions', async (req, res) => {
    ...
    await runFallbackLoop({
        ...
        route: () => routeRequest(estimatedTotal, state.skipKeys.size > 0 ? state.skipKeys : undefined, preferredModel, false, false, state.skipModels.size > 0 ? state.skipModels : undefined, groupChain ?? resolvedChain?.chain),
    ...
    await runFallbackLoop({
        ...
        dispatch: async (route, attempt) => {
            ...
            if (stream) {
                ...
                try {
                    const gen = route.provider.streamChatCompletion(route.apiKey, outboundMessages, route.modelId, { temperature, max_tokens, top_p, stop, tools, tool_choice, parallel_tool_calls, ...samplingParams }, quotaContextForRoute(route, 'chat/completions'));
                    for await (const chunk of gen) {
                        if (clientGone)
                            break; // client hung up: stop pulling; reader.cancel() aborts upstream
                ...
                const result = await route.provider.chatCompletion(route.apiKey, outboundMessages, route.modelId, { temperature, max_tokens, top_p, stop, tools, tool_choice, parallel_tool_calls, ...samplingParams }, quotaContextForRoute(route, 'chat/completions'));
