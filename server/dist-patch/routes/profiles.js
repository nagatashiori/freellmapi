...
const profileNameSchema = z
    ...
    .refine((name) => !RESERVED_PROFILE_NAMES.includes(name.toLowerCase()), 'This name is reserved by the system');
...
profilesRouter.get('/active', (_req, res) => {
    const db = getDb();
    const row = db.prepare(`SELECT value FROM settings WHERE key = 'active_profile_id'`).get();
...
profilesRouter.post('/active', (req, res) => {
    const db = getDb();
...
profilesRouter.get('/:id/models', (req, res) => {
    const db = getDb();
...
profilesRouter.post('/', (req, res) => {
    ...
    const duplicate = db.prepare('SELECT id FROM profiles WHERE LOWER(name) = LOWER(?)').get(name);
    if (duplicate) {
        res.status(409).json({ error: { message: `Profile with name '${name}' already exists` } });
        return;
    }
    const maxOrder = db.prepare('SELECT COALESCE(MAX(sort_order), 0) AS mx FROM profiles').get().mx;
    let layoutConfig = null;
    let autoSort = null;
    if (sourceProfileId) {
        const source = db.prepare('SELECT layout_config, auto_sort FROM profiles WHERE id = ?').get(sourceProfileId);
        if (source) {
            layoutConfig = source.layout_config;
            autoSort = source.auto_sort;
        }
    }
    const result = db.prepare('INSERT INTO profiles (name, emoji, color, type, sort_order, layout_config, auto_sort) VALUES (?, ?, ?, ?, ?, ?, ?)').run(name, emoji, color, 'custom', maxOrder + 1, layoutConfig, autoSort);
    const profileId = result.lastInsertRowid;
    if (sourceProfileId) {
        const source = db.prepare('SELECT id FROM profiles WHERE id = ?').get(sourceProfileId);
        if (!source) {
...
function copyFromDefault(db, profileId) {
    db.prepare(`
    INSERT INTO profile_models (profile_id, model_db_id, priority, enabled)
    SELECT ?, model_db_id, priority, enabled
...
profilesRouter.put('/:id', (req, res) => {
    const db = getDb();
    const profileId = getId(req);
    const profile = db.prepare('SELECT id, type FROM profiles WHERE id = ?').get(profileId);
    if (!profile) {
        res.status(404).json({ error: { message: 'Profile not found' } });
        return;
    }
    ...
    if (parsed.data.name !== undefined) {
        const duplicate = db.prepare('SELECT id FROM profiles WHERE LOWER(name) = LOWER(?) AND id != ?').get(parsed.data.name, profileId);
        if (duplicate) {
...
profilesRouter.put('/:id/reorder', (req, res) => {
    const db = getDb();
    const profileId = getId(req);
    const profile = db.prepare('SELECT id FROM profiles WHERE id = ?').get(profileId);
    if (!profile) {
...
profilesRouter.post('/:id/reset', (req, res) => {
    const db = getDb();
    const profileId = getId(req);
    const profile = db.prepare('SELECT id FROM profiles WHERE id = ?').get(profileId);
    if (!profile) {
...
profilesRouter.delete('/:id', (req, res) => {
    ...
    const activeRow = db.prepare(`SELECT value FROM settings WHERE key = 'active_profile_id'`).get();
    const activeId = activeRow ? parseInt(activeRow.value) : null;
    db.prepare('DELETE FROM profiles WHERE id = ?').run(profileId);
    if (activeId === profileId) {
        const defaultProf = db.prepare("SELECT id FROM profiles WHERE type = 'default' OR type = 'builtin' ORDER BY id ASC LIMIT 1").get();
        const fallbackId = defaultProf?.id ?? db.prepare('SELECT id FROM profiles ORDER BY sort_order ASC LIMIT 1').get()?.id;
        if (fallbackId) {
...
function sortProfileModels(db, profileId, preset) {
    let models = [];
    if (preset === 'budget') {
...
profilesRouter.post('/:id/sort/:preset', (req, res) => {
    const db = getDb();
    const profileId = getId(req);
    const profile = db.prepare('SELECT id FROM profiles WHERE id = ?').get(profileId);
    if (!profile) {
...
export function seedProfiles(db) {
    const count = db.prepare("SELECT COUNT(*) as cnt FROM profiles WHERE type = 'default' OR type = 'builtin'").get();
    if (count.cnt > 0)
        return;
    const builtins = [
        {
    ...
    const seed = db.transaction(() => {
        for (const builtin of builtins) {
            ...
            const profileId = result.lastInsertRowid;
            const models = db.prepare('SELECT id, LOWER(display_name) as name FROM models ORDER BY id ASC').all();
