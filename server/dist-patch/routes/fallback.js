...
fallbackRouter.get('/penalty-inspector', (_req, res) => {
...
fallbackRouter.get('/token-usage', (_req, res) => {
  ...
  `).all();
    ...
    const keyCountMap = new Map(db.prepare("SELECT platform, COUNT(*) as count FROM api_keys WHERE enabled = 1 AND status IN ('healthy', 'unknown') GROUP BY platform").all()
