...
mediaRouter.post('/custom', (req, res) => {
    ...
    `).get(modelId);
        const priority = existingModel && existingModel.modality === parsed.data.modality
            ...
            : db.prepare('SELECT COALESCE(MAX(priority), 0) AS maxPriority FROM media_models WHERE modality = ?')
                .get(parsed.data.modality).maxPriority + 1;
        if (existingModel) {
            db.prepare(`
        UPDATE media_models
           SET display_name = ?,
...
mediaRouter.delete('/custom/:id', (req, res) => {
    const id = Number(req.params.id);
    if (!Number.isInteger(id)) {
        res.status(400).json({ error: { message: 'Invalid id' } });
        return;
    ...
    const db = getDb();
    const row = db.prepare("SELECT key_id FROM media_models WHERE id = ? AND platform = 'custom'").get(id);
    if (!row) {
