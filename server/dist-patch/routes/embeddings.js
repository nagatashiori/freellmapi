...
import { listEmbeddingModels, getDefaultFamily, probeEmbeddingDimensions, EmbeddingsError, } from '../services/embeddings.js';
...
embeddingsRouter.get('/', (_req, res) => {
    ...
    const customKeyIds = new Set(db.prepare("SELECT id FROM api_keys WHERE platform = 'custom' AND enabled = 1 AND status IN ('healthy', 'unknown')").all().map(r => r.id));
...
function decryptExistingKey(row) {
    if (!row)
        return null;
    try {
...
embeddingsRouter.post('/custom', async (req, res) => {
  ...
  `).get(family, modelId);
    ...
    const upsert = db.transaction(() => {
      ...
      SELECT id, priority
       ...
       LIMIT 1
    `).get(modelId);
        const priority = existingModel?.priority ?? (db.prepare('SELECT COALESCE(MAX(priority), 0) AS maxPriority FROM embedding_models WHERE family = ?')
            .get(family).maxPriority + 1);
        if (existingModel) {
            db.prepare(`
...
embeddingsRouter.delete('/custom/:id', (req, res) => {
    ...
    const db = getDb();
    const row = db.prepare("SELECT family, key_id FROM embedding_models WHERE id = ? AND platform = 'custom'").get(id);
    if (!row) {
