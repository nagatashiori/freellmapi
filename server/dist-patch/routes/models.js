...
import { isCatalogManagedModel, recordCatalogModelTombstone, upsertModelOverrides, } from '../services/model-state.js';
...
function dbValue(key, value) {
    if (key === 'enabled' || key === 'supportsVision' || key === 'supportsTools')
        return value ? 1 : 0;
    return value;
}
function fetchModelRow(id) {
    return getDb()
...
modelsRouter.delete('/custom/:id', (req, res) => {
...
modelsRouter.patch('/:id', (req, res) => {
    ...
    const modelPatch = { ...parsed.data };
    delete modelPatch.fallbackEnabled;
    const modelKeys = Object.keys(modelPatch);
    ...
    const applyUpdate = db.transaction(() => {
        if (modelKeys.length > 0) {
            ...
            for (const key of modelKeys) {
                assignments.push(`${MODEL_FIELD_COLUMNS[key]} = ?`);
                values.push(dbValue(key, modelPatch[key]));
            }
            values.push(id);
            ...
            if (isCatalogManagedModel(row)) {
                ...
                for (const key of [
                    ...
                    if (Object.prototype.hasOwnProperty.call(modelPatch, key)) {
                        overridePatch[key] = modelPatch[key];
                    }
                }
